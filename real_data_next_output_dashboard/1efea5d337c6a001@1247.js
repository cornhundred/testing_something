...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([["observable_inputs_slice-1_replicate-2.json",new URL("./files/dcff2df9d365be5fa9515289e89cab9a4e38896e542f877a58dc9b72ed8aba4069c4ea9cc1b5386a1a335866db40bcca4fc65776364114fb462626a6769b8145",import.meta.url)],["clustergrammer-gl.0.22.0.packd.js",new URL("./files/7d6cecc3bad2ffc762b6a03b9f88ecb2a89dc2073195325a3eadb957a7f9cf8948047e657716fca537b05105f550fefd6203135db3045e9d12497f72efa65b58",import.meta.url)],["leiden_cgm@1.json",new URL("./files/61013c02a5925e14b8212754aea238e372d13b7a8572bc6f9c659309bb6bb712a07a3e40e8cb7d1d0e184dd9291a6e8db99e46cab52bb45e281f6d8659f087d1",import.meta.url)]]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], function(md){return(
md`# Cells, Transcripts, Polygons, Clicking, Heatmap`
)});
  main.variable(observer()).define(["dendro_col_selection"], function(dendro_col_selection){return(
dendro_col_selection
)});
...
md`# Deck.gl`
  ...
  main.variable(observer()).define(["dendro_col_selection"], function(dendro_col_selection){return(
dendro_col_selection.length
)});
  main.variable(observer()).define(["cell_data"], function(cell_data){return(
cell_data[0]
)});
  main.variable(observer("cell_layer")).define("cell_layer", ["deck","cell_data","apply_map_morph","select_meta","dendro_col_selection","d3","cat_colors","cat_opacity","opacity_scale","gene_data","radius","is_pickable","radius_min_pixels","map_type","transitions"], function(deck,cell_data,apply_map_morph,select_meta,dendro_col_selection,d3,cat_colors,cat_opacity,opacity_scale,gene_data,radius,is_pickable,radius_min_pixels,map_type,transitions){return(
new deck.ScatterplotLayer({
      ...
      updateTriggers: {
        getFillColor: [select_meta, dendro_col_selection],
...
require(await FileAttachment("clustergrammer-gl.0.22.0.packd.js").url())
)});
  main.variable(observer("network")).define("network", ["FileAttachment"], function(FileAttachment){return(
FileAttachment("leiden_cgm@1.json").json()
)});
  ...
  main.variable(observer("viewof cgm")).define("viewof cgm", ["DOM","html","CGM","network","reactiveValue","d3"], function*(DOM,html,CGM,network,reactiveValue,d3)
{
  ...
  main.variable(observer("dendro_col_selection")).define("dendro_col_selection", ["Generators","cgm","mutable select_meta","viewof cgm"], function(Generators,cgm,$0,$1){return(
Generators.observe(notify => {
  ...
  const mousemove = () => {
    ...
    if (cgm.params.tooltip.tooltip_type.includes('dendro')){
      $0.value = 'leiden'
      console.log('select_meta to leiden from dendro click')
  ...
  $1.addEventListener("click", mousemove);
  notify(cgm.params.dendro.selected_clust_names.map(x => x.replace('Leiden-', '')));

  return () => $1.removeEventListener("mousemove", mousemove);
})
)});
  main.variable(observer("heatmap_row")).define("heatmap_row", ["Generators","cgm","mutable select_meta","d3","viewof cgm"], function(Generators,cgm,$0,d3,$1){return(
Generators.observe(notify => {
  const mouse_click = () => {
    ...
    if (cgm.params.tooltip.tooltip_type === 'row-label'){
      ...
      // if (d3.select('#meta_dropdown').select('select').node().value !== inst_name){
      $0.value = inst_name
      // console.log('select_meta to inst_name from heatmap click')
      d3.select('#meta_dropdown').select('select').node().value = inst_name
      // } else {
      //   mutable select_meta = 'leiden'
      //   console.log('select_meta to leiden from clicking heatmap row that is equal to the value in the dropdown')
      //   d3.select('#meta_dropdown').select('select').node().value = 'leiden'
      // }  
  ...
  $1.addEventListener("click", mouse_click);
  
  notify(cgm.params.int.mouseover.row.name);

  return () => $1.removeEventListener("mousemove", mouse_click);
})
)});
