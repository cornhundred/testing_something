...
md`# Deck.gl Jupyter Spatial and UMAP - Rotated Video (gene loading)

https://colab.research.google.com/drive/1lXDh_AFgx3m9Qp307nz--1UwoBDkfbtH?usp=sharing

switched to import {slider, select} from '@jashkenas/inputs'
`
)});
...
select_meta
  ...
  main.variable(observer()).define(["gene_max"], function(gene_max){return(
gene_max
)});
  main.variable(observer()).define(["select_meta"], function(select_meta){return(
['none', 'leiden'].includes(select_meta)
)});
...
html`
  ...
  <div style="display: flex;">
   <div style="margin-left: 15px; margin-right: -30px; border:1px; text-align:right;">${$0} </div>
...
select_meta
  ...
  main.variable(observer()).define(["data","select_meta"], function(data,select_meta){return(
data[0][select_meta]
)});
...
FileAttachment("observable_inputs_slice-1_replicate-2@1.json").json()
  ...
  main.variable(observer()).define(["map_type","umap_x_scale","umap_y_scale","transitionDuration","d3","deck","data","select_meta","cat_colors","cat_opacity","opacity_scale","gene_data","radius","radius_min_pixels","deckgl"], function(map_type,umap_x_scale,umap_y_scale,transitionDuration,d3,deck,data,select_meta,cat_colors,cat_opacity,opacity_scale,gene_data,radius,radius_min_pixels,deckgl)
{ 
...
meta_data_cols_ini.concat(gene_names)
  ...
  main.variable(observer()).define(["meta_data_cols"], function(meta_data_cols){return(
meta_data_cols
)});
  main.variable(observer()).define(["select_meta"], function(select_meta){return(
select_meta
...
Math.max.apply(Math, gene_data.map(function(x) { return parseInt(x[select_meta]) }))
)});
...
inputs['cat_colors']
  ...
  main.variable(observer()).define(["d3","cat_colors"], function(d3,cat_colors){return(
...
data[0]
  ...
  main.variable(observer("tooltip")).define("tooltip", ["meta_data_cols"], function(meta_data_cols){return(
meta_data_cols
)});
  main.variable(observer()).define(["tooltip"], function(tooltip){return(
tooltip
)});
  main.variable(observer("tooltip_string")).define("tooltip_string", ["tooltip","data"], function(tooltip,data){return(
tooltip.map(x => x + ':' + String(data[0][x]) ).join('\n')
)});
  main.variable(observer()).define(["data"], function(data){return(
data[0]['name']
)});
  ...
  main.variable(observer("gene_data")).define("gene_data", ["select_meta","d3","gene_data_url"], function(select_meta,d3,gene_data_url)
{
  ...
  main.variable(observer()).define(["gene_data"], function(gene_data){return(
gene_data
)});
  main.variable(observer()).define(["data"], function(data){return(
data
