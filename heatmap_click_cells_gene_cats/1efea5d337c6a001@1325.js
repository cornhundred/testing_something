...
cell_data[0]
  ...
  main.variable(observer("cell_layer")).define("cell_layer", ["deck","cell_data","apply_map_morph","select_meta","selected_leidens","d3","cat_colors","cat_opacity","opacity_scale","gene_data","radius","is_pickable","radius_min_pixels","map_type","transitions","mutable selected_leidens","mutable select_meta"], function(deck,cell_data,apply_map_morph,select_meta,selected_leidens,d3,cat_colors,cat_opacity,opacity_scale,gene_data,radius,is_pickable,radius_min_pixels,map_type,transitions,$0,$1){return(
...
require(await FileAttachment("clustergrammer-gl@1.0.22.0.packd.js").url())
)});
...
[]
  ...
  main.variable(observer("col_selection")).define("col_selection", ["Generators","cgm","mutable select_meta","mutable selected_leidens","viewof cgm"], function(Generators,cgm,$0,$1,$2){return(
Generators.observe(notify => {
  ...
  const mousemove = () => {
    ...
    if (cgm.params.tooltip.tooltip_type.includes('dendro')){
      ...
      $1.value = cgm.params.dendro.selected_clust_names.map(x => x.replace('Leiden-', ''))
    } 
      // else {
    //   // notify([])
  ...
  main.variable(observer("heatmap_col")).define("heatmap_col", ["Generators","cgm","mutable select_meta","mutable selected_leidens","viewof cgm"], function(Generators,cgm,$0,$1,$2){return(
Generators.observe(notify => {
  const mouse_click = () => {
    ...
    if (cgm.params.tooltip.tooltip_type === 'col-label'){
      ...
      $0.value = 'leiden'
      $1.value = [cgm.params.int.mouseover.col.name.replace('Leiden-', '')] 
